...
deployments_data_clean <- deployments_data_raw |>  
  ...
  mutate(
    ...
    deployment_duration = as.numeric(difftime(end_date, start_date, units = "days"))) |> 
      filter(deployment_id != "L56" #L56 was a deployment with no cameras/sequences
  )
...
deployments_data_clean <- deployments_data_clean %>%
  mutate(study_part = 2) |>  ## tags all of these sequences as part of the second part of the study)
...
#join hibernacula_coordinates to february_study_observations
...
february_study_observations <- february_study_observations |> 
  ...
  ungroup() |> 
filter(!(placename == "H41" & camera_name == "C6"), ## removing this deployment because camera was set improperly (bad angle)
       !(placename == "H23" & camera_name == "C3"),  ## removing this deployment because camera was set improperly (bad angle)
       !(placename == "H24" & camera_name == "C1")) |>  ## removing this deployment because it seems to have failed on day 1
  select(-calculated_deployment_duration)
...
## REMOVING OBSERVATIONS BY DIFFERENT CAMERAS AT THE SAME PLACE, DATE, AND MINUTE 
  ...
  overlap_pairs_part_1 <- february_study_observations |> 
    inner_join(february_study_observations, by = c("placename", "obs_start_date", "truncated_time", "common_name")) |> 
    filter(
  ...
  deduplicated_february_observations <- february_study_observations |> 
    filter(!row_id %in% rows_to_remove_part_1)

  
## analyze how long each February deployment lasted
february_placename_summary <- february_study_observations |> 
...
## full join february_placename_summary and deployment_data_clean to get all deployments in one spot
combined_deployments_summary <- february_placename_summary |> 
  full_join(deployments_data_clean, by = c("placename", "camera_name","start_date", "end_date", "deployment_duration", "feature_type_methodology", "habitat_type", "trail", "study_part","latitude","longitude"))
...
february_reduced_to_hourly <- deduplicated_february_observations |>
...
## Working with February Study Data
```{r}
#| eval: false ### RUNNING THIS BLOCK INCORPORATES FEBRUARY DATA INTO THE REST OF THE ANALYSIS. Need coordinates for the February data as of 7/17/2025.
#| include: false

# Find common columns
common_cols <- intersect(names(february_study_observations), names(merged_data_with_deduplicated_observations))

# Perform join by only the shared columns
merged_data_with_deduplicated_observations <- full_join(february_study_observations, 
