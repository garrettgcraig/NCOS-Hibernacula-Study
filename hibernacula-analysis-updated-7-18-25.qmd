...
camera_effort_by_feature_combined <- bind_rows(
  ...
  group_by(feature_type_methodology) %>%
  summarize(
    total_camera_hours = sum(total_camera_hours, na.rm = TRUE),
    .groups = "drop"
  )
...
plots <- lapply(feature_types, function(ft) {
  ggplot(
    ...
    labs(
      title = paste("Species Detection Rates:", ft),
      x = "Residency Status",
...
```
```{r}
# For each feature type, print resident species
resident_species_by_feature <- species_feature_detections %>%
  filter(residency_status == "resident") %>%
  group_by(feature_type_methodology) %>%
  summarise(resident_species = paste(sort(unique(common_name)), collapse = ", "))

print(resident_species_by_feature)
```
