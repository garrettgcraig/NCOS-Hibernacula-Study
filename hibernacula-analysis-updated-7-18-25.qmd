...
daily_site_summary <- clean_study_part_2_joined_sequences_and_deployments %>%
  group_by(
...
full_daily_data <- expanded_deployments_study_part_2 %>%
...
# Perform the t-test
stats_t_test_result <- t.test(daily_total_observations ~ feature_type_methodology_recoded, data = daily_site_summary)
...
```{r}
# 1 predictor models
nb_model_1a <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded, data = daily_site_summary)
nb_model_1b <- MASS::glm.nb(daily_total_observations ~ trail, data = daily_site_summary)
nb_model_1c <- MASS::glm.nb(daily_total_observations ~ habitat_type, data = daily_site_summary)
nb_model_1d <- MASS::glm.nb(daily_total_observations ~ notable_entrances, data = daily_site_summary)

# 2 predictor models
nb_model_2a <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + trail, data = daily_site_summary)
nb_model_2b <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + habitat_type, data = daily_site_summary)
nb_model_2c <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + notable_entrances, data = daily_site_summary)
nb_model_2d <- MASS::glm.nb(daily_total_observations ~ trail + habitat_type, data = daily_site_summary)
nb_model_2e <- MASS::glm.nb(daily_total_observations ~ trail + notable_entrances, data = daily_site_summary)
nb_model_2f <- MASS::glm.nb(daily_total_observations ~ habitat_type + notable_entrances, data = daily_site_summary)

# 3 predictor models
nb_model_3a <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + trail + habitat_type, data = daily_site_summary)
nb_model_3b <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + trail + notable_entrances, data = daily_site_summary)
nb_model_3c <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + habitat_type + notable_entrances, data = daily_site_summary)
nb_model_3d <- MASS::glm.nb(daily_total_observations ~ trail + habitat_type + notable_entrances, data = daily_site_summary)

# 4 predictor model (full model)
nb_model_4 <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + trail + habitat_type + notable_entrances, data = daily_site_summary)
...
p_value_text <- round(stats_t_test_result$p.value, 5)
mean_hibernacula <- round(stats_t_test_result$estimate[1], 2)
mean_log_boulder <- round(stats_t_test_result$estimate[2], 2)
mean_difference <- round(diff(stats_t_test_result$estimate), 2)
ci_lower <- round(stats_t_test_result$conf.int[1], 2)
ci_upper <- round(stats_t_test_result$conf.int[2], 2)

# Cohen's d
cohen_d_value <- round(stats_cohen_d_result$estimate, 2)
cohen_d_magnitude <- stats_cohen_d_result$magnitude
cohen_d_ci_lower <- round(stats_cohen_d_result$conf.int[1], 2)
cohen_d_ci_upper <- round(stats_cohen_d_result$conf.int[2], 2)

# Shapiro-Wilk normality
shapiro_p_hibernacula <- round(stats_normality_test$shapiro_p_value[stats_normality_test$feature_type_methodology_recoded == "Constructed Hibernacula"], 4)
shapiro_p_log_boulder <- round(stats_normality_test$shapiro_p_value[stats_normality_test$feature_type_methodology_recoded == "Log/Boulder"], 4)

# Levene’s test
levene_f_value <- round(stats_levene_test$"F value"[1], 2)
levene_p_value <- round(stats_levene_test$"Pr(>F)"[1], 4)
```
...
Trail proximity and Habitat Type (except Scrub) effects appear meaningful in the negative binomial model.
...
camera_effort_by_feature_combined <- bind_rows(
    camera_effort_by_feature_study_part_1 %>% mutate(study_part = "part_1"),
    camera_effort_by_feature_study_part_2 %>% mutate(study_part = "part_2"))
...
hourly_summary_with_effort_study_part_2$hour_am_pm <- factor(hourly_summary$hour_am_pm, 
...
species_abundance <- clean_study_part_2_joined_sequences_and_deployments |> 
  group_by(feature_type_methodology, class, family, genus_species, common_name) |> 
  summarise(abundance = n(), .groups = 'drop')

# Plot for Aves
figs_aves_plot <- ggplot(filter(species_abundance, class == "Aves"), 
...
# Plot for Mammalia
figs_mammalia_plot <- ggplot(filter(species_abundance, class == "Mammalia"), 
                        aes(x = common_name, y = abundance, fill = feature_type_methodology)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_brewer(palette = "Set2") +  # More distinct colors
...
figs_reptilia_plot <- ggplot(filter(species_abundance, class == "Reptilia"), 
                        aes(x = common_name, y = abundance, fill = feature_type_methodology)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_brewer(palette = "Set2") +  # More distinct colors
