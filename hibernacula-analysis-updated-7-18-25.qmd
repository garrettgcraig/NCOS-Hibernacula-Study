...
study_part_1_placename_summary <- clean_study_part_1_observations |> 
  ...
  group_by(
    ...
    feature_type_methodology, habitat_type, trail, study_part,latitude,longitude,count_of_obs_dates_at_place
  ) |> 
...
hourly_presence_february <- clean_study_part_1_observations |>
  ...
  group_by(placename, obs_day, obs_hour, feature_type_methodology,common_name,habitat_type,trail,study_part,start_date,end_date,deployment_duration) |> 
...
combined_deployments_summary <- study_part_1_placename_summary |> 
  full_join(clean_study_part_2_deployments, by = c("placename", "camera_name","start_date", "end_date", "deployment_duration", "feature_type_methodology", "habitat_type", "trail", "study_part","latitude","longitude"))
...
# Plot
ggplot(hourly_presence_summary, aes(x = obs_hour, y = species_detections, fill = feature_type_methodology)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ study_part
             ) +
  scale_x_continuous(breaks = 0:23) +
  labs(
    title = "Hourly Wildlife Presence by Feature Type and Study Period",
    x = "Hour of Day",
    y = "Number of Species Detections",
    fill = "Feature Type"
  ) +
  theme_minimal()
...
```{r}
# Model 1: Feature type only
nb_model_1 <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded, data = daily_site_summary)

# Model 2: Feature type + trail
nb_model_2 <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + trail, data = daily_site_summary)

# Model 3: Feature type + trail + habitat_type
nb_model_3 <- MASS::glm.nb(daily_total_observations ~ feature_type_methodology_recoded + trail + habitat_type, data = daily_site_summary)
...
stargazer(lm_model_1, lm_model_2, lm_model_3,
          ...
          dep.var.labels = "Daily Total Observations",
          column.labels = c("Model 1", "Model 2", "Model 3"),
          covariate.labels = c("Feature Type - Boulder/Log", "Trail - Yes", 
                               "Habitat Type - Grassland", "Habitat Type - Scrub","Reference"),
          digits = 3,
          notes = "Reference levels: Constructed Hibernacula (Feature Type), No (Trail), Marsh (Habitat Type).",
          no.space = TRUE)
```
...
Trail proximity and Habitat Type effects appear meaningful in the Poisson models.
And the Poisson models fit the data better than linear models, but overdispersion must be addressed.
Quasi-Poisson corrects for this with corrected standard errors, and the Quasi-Poisson Model 3 that includes all three variables (feature type, trail proximity, and habitat type) shows significance (p \< 0.05) for all variables except for Scrub habitat type, which is no longer significant due to larger standard errors.
