...
if (!is.null(model_avg_result)) {
  ...
  avg_display <- cond_coefs %>%
    ...
    mutate(
      Predictor = case_when(
        Coefficient == "(Intercept)" ~ "Intercept (Artificial Refuge, Marsh, Interior)",
        ...
        Coefficient == "habitat_typeGrassland" ~ "Grassland",
  ...
  avg_display %>%
    ...
    kable(
      ...
      digits = 3,
      caption = sprintf("Model-Averaged Coefficients (conditional averaging via MuMIn, %d competitive models, ΔAICc < 2). Unconditional SEs account for model selection uncertainty. Reference: Artificial Refuge, not trail-adjacent, marsh habitat. Ratio columns are blank for the intercept: exp(β) there is the baseline detection rate (%.3f per camera hour, %.1f per camera day) rather than a ratio.", length(top_models), baseline_rate_hr, baseline_rate_day),
...
cat("=== Moran's I Test for Spatial Autocorrelation (all competitive models) ===\n\n")

competitive_models <- sorted_models[competitive_idx]
...
if (all(moran_results$P_Value >= 0.05)) {
  cat("No significant spatial autocorrelation detected in any competitive model.\n")
...
kable(display_data %>% dplyr::select(-row_type_flag),
      ...
      caption = "Table 1. Negative Binomial Model Comparison (AICc). Models ranked by ΔAICc; coefficients shown as β, IRR, and % Change. Reference: Artificial Refuge, not trail-adjacent, marsh habitat. IRR and % Change are blank for intercept rows, where exp(β) is the baseline rate rather than a ratio.",
      align = c("l", rep("r", 7)),
      digits = 3) %>%
...
kable(top_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 2. Competitive Models (ΔAICc < 2). Preferred (lowest AICc, highest weight): %s. Reference: Artificial Refuge, not trail-adjacent, marsh habitat. IRR and %% Change are blank for intercept rows, where exp(β) is the baseline rate rather than a ratio.", best_model_name),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
  ...
  if (nrow(stability_data) > 0) {
    cv_summary <- stability_data %>%
      ...
      mutate(Predictor = case_when(
        ...
        Predictor == "habitat_typeGrassland" ~ "Grassland Habitat",
...
### DHARMa Diagnostics for Competitive Models
```{r}
#| echo: false
#| fig-width: 10
#| fig-height: 4
...
cat("=== DHARMa Residual Diagnostics for All Models with ΔAICc < 2 ===\n\n")

for (i in seq_along(top_models)) {
  mod_name <- names(top_models)[i]
  mod <- top_models[[i]]

  cat(sprintf("--- %s (ΔAICc = %.2f, w = %.3f, df = %d) ---\n",
              mod_name,
              sorted_delta_aicc[top_idx[i]],
              sorted_weights[top_idx[i]],
              attr(logLik(mod), "df")))
...
kable(int_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 3. Interior Sites Only (n=%d). Models ranked by ΔAICc. Reference: Artificial Refuge, marsh habitat. IRR and %% Change are blank for intercept rows, where exp(β) is the baseline rate rather than a ratio.", nrow(interior_data)),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
...
ggsave(here("figures", "FINAL_fig1_camera_station_map.png"), plot = fig_final_1,
       width = 10, height = 10.7, dpi = 300, bg = "white")
```

## Figure 3. Detection Rates by Feature Type

```{r fig-final-3-detection-rates}
#| echo: false
#| fig-cap: "**Figure 3. Detection rates by feature type.** Average number of wildlife detections per camera trap day across the three feature types in the spring study. Open points denote trail-adjacent (edge) features; filled points denote interior features. Black diamonds indicate mean values and vertical lines show 95% confidence intervals."
#| fig-width: 8
#| fig-height: 6
...
fig3_data <- placename_summary_pt_2 %>%
...
ggsave(here("figures", "FINAL_fig3_detection_rates_by_feature_type.png"),
       plot = fig_final_3, width = 8, height = 6, dpi = 300, bg = "white")
```

## Figure 4. Species Detection Rates by Habitat Type

```{r fig-final-4-species-habitat}
#| echo: false
#| fig-cap: "**Figure 4. Spring study species detection rates per camera day by habitat type.** Species with more than 50 detections in the study period are displayed individually. All panels share a common y-axis scale."
#| fig-width: 7
#| fig-height: 12
...
fig_final_4 <- ggplot(resident_habitat_data,
                      aes(x = habitat_type, y = detections_per_camera_day, fill = habitat_type)) +
  geom_col(width = 0.7) +
...
fig_final_5 <- wrap_plots(final_residency_plots, guides = "collect") &
...
ggsave(here("figures", "FINAL_fig5_species_residency_by_feature_type.png"),
       plot = fig_final_5, width = 13, height = 7, dpi = 300, bg = "white")
```
...
top_display %>%
  ...
  kable(
    col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
    digits = 3,
    caption = sprintf("Table 1. Negative binomial models within ΔAICc < 2 of the top-ranked model (n = %d camera stations). Models are ranked by ΔAICc; Akaike weights (w) give the relative probability that each model is the best approximating model in the candidate set. Coefficients are shown as β, incidence rate ratios (IRR), and %% change relative to the reference condition. All models include log(camera hours) as an offset. Reference conditions: artificial refuge, not trail-adjacent, marsh habitat. IRR and %% change are blank for intercept rows, where exp(β) is the baseline detection rate per camera hour rather than a ratio.", nrow(placename_summary_pt_2)),
