---
title: "NCOS Constructed Refugia Study - Revisions"
...
raw_pt_2_dep$feature_type_methodology[raw_pt_2_dep$feature_type_methodology == "Constructed Hibernacula"] <- "Constructed Refugium"
  ...
  mutate(
    ...
    feature_type_methodology_recoded = factor(
      case_when(
        ...
        TRUE ~ "Constructed Refugium"
      ),
      levels = c("Constructed Refugium", "Boulder", "Log")
    ),
...
#   4) Occam's razor: prefer most parsimonious model within ΔAICc < 2
# ============================================================================
...
cat("Interaction model convergence:\n")
cat("  Feature Type x Habitat:", ifelse(!is.null(nb_offset_int_ft_hab), "converged", "FAILED"), "\n")
cat("  Feature Type x Trail:", ifelse(!is.null(nb_offset_int_ft_trail), "converged", "FAILED"), "\n")
cat("  Feature Type x Trail + Habitat:", ifelse(!is.null(nb_offset_int_ft_trail_hab), "converged", "FAILED"), "\n")
```
...
# Add interaction models if they converged
if (!is.null(nb_offset_int_ft_hab))
  all_models[["Feature Type x Habitat Type"]] <- nb_offset_int_ft_hab
if (!is.null(nb_offset_int_ft_trail))
  all_models[["Feature Type x Trail"]] <- nb_offset_int_ft_trail
if (!is.null(nb_offset_int_ft_trail_hab))
...
# Identify best model applying Occam's razor: among models within ΔAICc < 2,
# prefer the most parsimonious (fewest df)
competitive_idx <- which(sorted_delta_aicc < 2)
competitive_df <- sapply(sorted_models[competitive_idx], function(m) attr(logLik(m), "df"))
best_idx <- competitive_idx[which.min(competitive_df)]
...
cat(sprintf("\nPreferred model (most parsimonious within ΔAICc < 2): %s\n", best_model_name))

top_weight_idx <- competitive_idx[which.max(sorted_weights[competitive_idx])]
top_weight_name <- names(sorted_models)[top_weight_idx]
top_weight_val <- sorted_weights[top_weight_idx]

cat("\n=== Akaike Weight Interpretation ===\n")
cat(sprintf("Highest Akaike weight among competitive models: %s (w = %.3f)\n", top_weight_name, top_weight_val))
if (top_weight_name != best_model_name) {
  cat(sprintf("Note: %s has the highest evidence ratio despite having more parameters than %s.\n", top_weight_name, best_model_name))
  cat("The Akaike weight represents the probability that a given model is the best approximating model\n")
  cat("in the candidate set. While parsimony favors the simpler model, the weight indicates the\n")
  cat("data provide stronger support for the more complex model.\n")
}

er <- top_weight_val / sorted_weights[competitive_idx[which(names(sorted_models)[competitive_idx] != top_weight_name)[1]]]
cat(sprintf("\nEvidence ratio (best weight / next competitive): %.2f\n", er))

cat("\n=== Model-Averaged Coefficients (weighted by Akaike weights) ===\n")
cat("Using full model averaging across the competitive set (ΔAICc < 2).\n")
...
top_weights_raw <- sorted_weights[top_idx]
top_weights_rescaled <- top_weights_raw / sum(top_weights_raw)

all_coef_names <- unique(unlist(lapply(top_models, function(m) names(coef(m)))))

avg_coefs <- data.frame(
  Coefficient = all_coef_names,
  Averaged_Beta = NA_real_,
  Averaged_IRR = NA_real_,
  N_Models = 0L,
  stringsAsFactors = FALSE
)

for (j in seq_along(all_coef_names)) {
  cn <- all_coef_names[j]
  beta_sum <- 0
  w_sum <- 0
  n_present <- 0L
  for (k in seq_along(top_models)) {
    cc <- coef(top_models[[k]])
    if (cn %in% names(cc)) {
      beta_sum <- beta_sum + cc[cn] * top_weights_rescaled[k]
      w_sum <- w_sum + top_weights_rescaled[k]
      n_present <- n_present + 1L
    }
  }
  avg_coefs$Averaged_Beta[j] <- beta_sum / w_sum
  avg_coefs$Averaged_IRR[j] <- exp(beta_sum / w_sum)
  avg_coefs$N_Models[j] <- n_present
}

print(avg_coefs, digits = 3)
...
# Extract deviance residuals from the best (Occam's razor) model
dev_resids <- residuals(best_model, type = "deviance")

# Moran's I test on deviance residuals
moran_result <- spdep::moran.test(dev_resids, listw)
cat("=== Moran's I Test for Spatial Autocorrelation ===\n")
cat(sprintf("Model tested: %s\n", best_model_name))
print(moran_result)

if (moran_result$p.value < 0.05) {
  cat("\n** Significant spatial autocorrelation detected (p < 0.05). **\n")
  cat("Consider adding spatial covariates (coordinates) or a spatial random effect.\n")
} else {
  cat("\n** No significant spatial autocorrelation detected. **\n")
  cat("Spatial independence assumption of the GLM is supported.\n")
}

# Also run Moran's I on the full additive model for comparison
dev_resids_full <- residuals(nb_offset_3a, type = "deviance")
moran_full <- spdep::moran.test(dev_resids_full, listw)
cat("\n=== Moran's I on full additive model (Feature Type + Trail + Habitat Type) ===\n")
print(moran_full)
```
...
```{r}
#| echo: false
# Reviewer flagged that 5 of 14 constructed refugia are on the periphery near trails
...
### Full Model Comparison Table (AICc)
```{r}
#| echo: false
...
kable(display_data %>% dplyr::select(-row_type_flag),
      ...
      caption = "Table 1. Negative Binomial Model Comparison (AICc). Models ranked by ΔAICc; coefficients shown as β, IRR, and % Change. Reference: Constructed Refugium, not trail-adjacent, marsh habitat.",
      align = c("l", rep("r", 7)),
      digits = 3) %>%
...
cat(sprintf("Preferred (most parsimonious): %s\n\n", best_model_name))
...
kable(top_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 2. Competitive Models (ΔAICc < 2). Preferred (most parsimonious): %s. Reference: Constructed Refugium, not trail-adjacent, marsh habitat.", best_model_name),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
...
interior_models <- interior_models[!sapply(interior_models, is.null)]
...
# Preferred model (parsimony within ΔAICc < 2)
int_competitive <- which(int_aicc$delta_AICc < 2)
int_competitive_df <- int_aicc$df[int_competitive]
int_best_idx <- int_competitive[which.min(int_competitive_df)]
int_best_name <- int_aicc$Model[int_best_idx]
int_best_model <- interior_models[[int_best_name]]

cat(sprintf("\nPreferred model (parsimony): %s\n", int_best_name))
cat("\nBest model summary:\n")
...
kable(int_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 3. Interior Sites Only (n=%d). Models ranked by ΔAICc. Reference: Constructed Refugium, marsh habitat.", nrow(interior_data)),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
...
#| fig-cap: "Average wildlife detections per camera trap day across all three feature type groups—constructed refugia, boulders, and logs. Points represent individual sites, black diamonds indicate mean values, and vertical lines show 95% confidence intervals."
...
p1 = ggplot(placename_summary_pt_2, aes(x = feature_type_methodology_recoded, y = detections_per_camera_day)) +
  ...
  scale_color_manual(values = c("Boulder" = "#66c2a5", "Log" = "#8da0cb", "Constructed Refugium" = "#fc8d62")) +
  labs(
...
figs_hourly_hibernacula <- pad_hourly(clean_pt_2_seq_dep_lumped, "Constructed Refugium")
hibernacula_effort <- feature_type_effort_summary_pt_2 |>
  dplyr::filter(feature_type_methodology == "Constructed Refugium") |>
  dplyr::pull(total_camera_days)
...
species_final_pt_2 <- species_combined_pt_2 %>%
  mutate(
    percent_hibernacula_visited = 100 * features_visited_at_constructed_refugium / total_features_pt_2_vec["Constructed Refugium"],
...
species_final_pt_2 <- species_final_pt_2 %>%
  mutate(
    rate_per_cam_day_hibernacula = count_observed_at_constructed_refugium / camera_days_vec["Constructed Refugium"],
...
t1 = species_final_pt_2 %>%
  ...
  dplyr::select(
    ...
    'Detections' = count_observed_at_constructed_refugium,
  ...
  mutate(
    ...
    '% Visited   ' = (
      (`% Visited` * total_features_pt_2_vec["Constructed Refugium"]) +
  ...
  add_header_above(c(" " = 3, "Constructed Refugia" = 3, "Boulders" = 3, "Logs" = 3, "Total" = 3))
...
feature_colors <- c(
  ...
  "Constructed Refugium" = brewer.pal(3, "Set2")[2],
...
```
### Map v 2 (Spring study — full NCOS boundary, shape-encoded features)
...
point_shapes_pt2 <- c("Constructed Refugium" = 15, "Boulder" = 2, "Log" = 1)
...
clean_pt_1_obs$feature_type_methodology[clean_pt_1_obs$feature_type_methodology == "Constructed Hibernacula"] <- "Constructed Refugium"
...
feature_types <- c("Constructed Refugium", "Boulder", "Log")
  ...
  map_site_sf <- map_site_sf %>%
    mutate(
      point_label = case_when(
        feature_type_methodology == "Constructed Refugium" & study_part_combined == "both" ~ "Refugium (both studies)",
        feature_type_methodology == "Constructed Refugium" ~ "Refugium (one study)",
        feature_type_methodology == "Boulder" ~ "Boulder",
        feature_type_methodology == "Log" ~ "Log",
        TRUE ~ as.character(feature_type_methodology)
      ),
      point_label = factor(point_label, levels = c(
        "Refugium (both studies)", "Refugium (one study)", "Boulder", "Log"
      ))
    )

  # Shape encoding: filled square=15, open square=0, triangle=2, circle=1
  point_shapes <- c(
    "Refugium (both studies)" = 15,
    "Refugium (one study)" = 0,
    "Boulder" = 2,
    "Log" = 1
  )
...
### Two-Panel Map (Winter / Spring)
```{r fig-deployment-map-2panel}
#| fig-cap: "**Camera trap deployment locations during winter (left) and spring (right) studies.** Point shape indicates feature type: square = constructed refugium, triangle = boulder, circle = log. Habitat types shown as semi-transparent overlays. Inset shows study location within California."
#| fig-width: 14
#| fig-height: 7
...
# Shape encoding
point_shapes_map <- c("Constructed Refugium" = 15, "Boulder" = 2, "Log" = 1)
...
p5 <- ggplot(placename_summary_pt_1_and_2, aes(x = feature_type_methodology, y = notable_entrances)) +
  ...
  scale_color_manual(values = c("Constructed Refugium" = "#fc8d62", "Boulder" = "#66c2a5", "Log" = "#8da0cb")) +
...
species_final_pt_1 <- species_combined_pt_1 %>%
  mutate(
    percent_hibernacula_visited = 100 * features_visited_at_constructed_refugium / total_features_pt_1_vec["Constructed Refugium"]
  )
...
species_final_pt_1 <- species_final_pt_1 %>%
  mutate(
    rate_per_cam_day_hibernacula = count_observed_at_constructed_refugium / camera_days_vec_pt_1["Constructed Refugium"]
  )
...
t1_pt1 = species_final_pt_1 %>%
  ...
  add_header_above(c(" " = 1, "Constructed Refugia" = 3))
...
species_final_hourly <- species_combined_hourly %>%
  mutate(
    percent_hibernacula_visited = 100 * features_visited_at_constructed_refugium / total_features_hourly_vec["Constructed Refugium"],
...
species_final_hourly <- species_final_hourly %>%
  mutate(
    rate_per_cam_day_hibernacula = count_observed_at_constructed_refugium / camera_days_vec_hourly["Constructed Refugium"],
...
t1_hourly <- species_final_hourly %>%
  ...
  mutate(
    'Detections   ' = Detections + `Detections ` + `Detections  `,
    '% Visited   ' = (
      (`% Visited` * total_features_hourly_vec["Constructed Refugium"]) +
...
species_final_hourly <- species_combined_hourly %>%
  mutate(
    ...
    percent_hibernacula_visited = 100 * safe_div(
      features_visited_at_constructed_refugium, total_features_constructed_refugium
    ),
    ...
    rate_per_cam_day_hibernacula = safe_div(
      count_observed_at_constructed_refugium, cam_days_constructed_refugium
    ),
    ...
    detections_total = rowSums(across(c(count_observed_at_constructed_refugium,
                                        count_observed_at_boulder,
                                        count_observed_at_log)), na.rm = TRUE),
    total_features_all = rowSums(across(c(total_features_constructed_refugium,
                                          total_features_boulder,
                                          total_features_log)), na.rm = TRUE),
    total_cam_days_all = rowSums(across(c(cam_days_constructed_refugium,
                                          cam_days_boulder,
                                          cam_days_log)), na.rm = TRUE),

    percent_visited_all = 100 * safe_div(
      rowSums(across(c(features_visited_at_constructed_refugium,
...
metrics_long <- species_final_hourly %>%
  transmute(
    ...
    # Artificial Refuge (AR)
    detections_hib = count_observed_at_constructed_refugium,
...
build_feature_spans <- function(part) {
  out <- c()
  n <- count_for(part, "hib"); if (n > 0) out <- c(out, "Constructed Refugium" = n)
