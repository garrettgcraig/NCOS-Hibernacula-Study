...
if (!is.null(model_avg_result)) {
  ...
  avg_display %>%
    ...
    kable(
      ...
      digits = 3,
      caption = sprintf("Model-Averaged Coefficients (conditional averaging via MuMIn, %d competitive models, ΔAICc < 2). Unconditional SEs account for model selection uncertainty. Reference: Artificial Refuge, not trail-adjacent, marsh habitat.", length(top_models)),
...
kable(display_data %>% dplyr::select(-row_type_flag),
      ...
      caption = "Table 1. Negative Binomial Model Comparison (AICc). Models ranked by ΔAICc; coefficients shown as β, IRR, and % Change. Reference: Artificial Refuge, not trail-adjacent, marsh habitat.",
      align = c("l", rep("r", 7)),
      digits = 3) %>%
...
kable(top_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 2. Competitive Models (ΔAICc < 2). Preferred (lowest AICc, highest weight): %s. Reference: Artificial Refuge, not trail-adjacent, marsh habitat.", best_model_name),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
...
kable(int_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 3. Interior Sites Only (n=%d). Models ranked by ΔAICc. Reference: Artificial Refuge, marsh habitat.", nrow(interior_data)),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
