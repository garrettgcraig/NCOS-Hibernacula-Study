---
title: "NCOS Artificial Refuge Study - Revisions"
...
raw_pt_2_dep$feature_type_methodology[raw_pt_2_dep$feature_type_methodology == "Constructed Hibernacula"] <- "Artificial Refuge (AR)"
...
clean_pt_2_seq_dep <- clean_pt_2_seq |> 
  ...
  mutate(
    ...
    feature_type_methodology_recoded = factor(
      ...
      levels = c("Artificial Refuge (AR)", "Boulder", "Log")
    )
  )
```
...
kable(display_data %>% dplyr::select(-row_type_flag),
      ...
      caption = "Table 1. Negative Binomial Model Comparison (AICc). Models ranked by ΔAICc; coefficients shown as β, IRR, and % Change. Reference: AR, not trail-adjacent, marsh habitat.",
      align = c("l", rep("r", 7)),
      digits = 3) %>%
...
kable(top_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 2. Competitive Models (ΔAICc < 2). Preferred (most parsimonious): %s. Reference: AR, not trail-adjacent, marsh habitat.", best_model_name),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
...
kable(int_display %>% dplyr::select(-row_type_flag),
      col.names = c("Model / Predictor", "β", "IRR", "% Change", "P-Value", "ΔAICc", "w", "RMSE"),
      caption = sprintf("Table 3. Interior Sites Only (n=%d). Models ranked by ΔAICc. Reference: AR, marsh habitat.", nrow(interior_data)),
      align = c("l", rep("r", 7)),
      digits = 3) %>%
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("striped", "hover", "condensed")) %>%
...
#| fig-cap: "Average wildlife detections per camera trap day across all two feature types groups—artificial refuges (AR) and logs/boulders. Points represent individual sites, black diamonds indicate mean values, and vertical lines show 95% confidence intervals."
...
p1 = ggplot(placename_summary_pt_2, aes(x = feature_type_methodology_recoded, y = detections_per_camera_day)) +
  ...
  scale_color_manual(values = c("Boulder" = "#66c2a5", "Log" = "#8da0cb", "Artificial Refuge (AR)" = "#fc8d62")) +
  labs(
...
figs_hourly_hibernacula <- pad_hourly(clean_pt_2_seq_dep_lumped, "Artificial Refuge (AR)")
hibernacula_effort <- feature_type_effort_summary_pt_2 |>
  dplyr::filter(feature_type_methodology == "Artificial Refuge (AR)") |>
  dplyr::pull(total_camera_days)
...
species_final_pt_2 <- species_combined_pt_2 %>%
  mutate(
    percent_hibernacula_visited = 100 * features_visited_at_artificial_refuge_ar / total_features_pt_2_vec["Artificial Refuge (AR)"],
...
species_final_pt_2 <- species_final_pt_2 %>%
  mutate(
    rate_per_cam_day_hibernacula = count_observed_at_artificial_refuge_ar / camera_days_vec["Artificial Refuge (AR)"],
...
t1 = species_final_pt_2 %>%
  ...
  dplyr::select(
    ...
    'Detections' = count_observed_at_artificial_refuge_ar,
  ...
  mutate(
    ...
    '% Visited   ' = (
      (`% Visited` * total_features_pt_2_vec["Artificial Refuge (AR)"]) +
  ...
  add_header_above(c(" " = 3, "Artificial Refuge (AR)" = 3, "Boulders" = 3, "Logs" = 3, "Total" = 3))
...
feature_colors <- c(
  ...
  "Artificial Refuge (AR)" = brewer.pal(3, "Set2")[2],
...
# Shape encoding: square=AR, triangle=boulder, circle=log
point_shapes_pt2 <- c("Artificial Refuge (AR)" = 15, "Boulder" = 2, "Log" = 1)
...
clean_pt_1_obs$feature_type_methodology[clean_pt_1_obs$feature_type_methodology == "Constructed Hibernacula"] <- "Artificial Refuge (AR)"
...
feature_types <- c("Artificial Refuge (AR)", "Boulder", "Log")
  ...
  map_site_sf <- map_site_sf %>%
    mutate(
      point_label = case_when(
        feature_type_methodology == "Artificial Refuge (AR)" & study_part_combined == "both" ~ "AR (both studies)",
        feature_type_methodology == "Artificial Refuge (AR)" ~ "AR (one study)",
        feature_type_methodology == "Boulder" ~ "Boulder",
        feature_type_methodology == "Log" ~ "Log",
        TRUE ~ as.character(feature_type_methodology)
      ),
      point_label = factor(point_label, levels = c(
        "AR (both studies)", "AR (one study)", "Boulder", "Log"
      ))
    )
...
#| fig-cap: "**Camera trap deployment locations during winter (left) and spring (right) studies.** Point shape indicates feature type: square = artificial refuge (AR), triangle = boulder, circle = log. Habitat types shown as semi-transparent overlays. Inset shows study location within California."
#| fig-width: 14
#| fig-height: 7
...
# Shape encoding
point_shapes_map <- c("Artificial Refuge (AR)" = 15, "Boulder" = 2, "Log" = 1)
...
p5 <- ggplot(placename_summary_pt_1_and_2, aes(x = feature_type_methodology, y = notable_entrances)) +
  ...
  scale_color_manual(values = c("Artificial Refuge (AR)" = "#fc8d62", "Boulder" = "#66c2a5", "Log" = "#8da0cb")) +
...
species_final_pt_1 <- species_combined_pt_1 %>%
  mutate(
    percent_hibernacula_visited = 100 * features_visited_at_artificial_refuge_ar / total_features_pt_1_vec["Artificial Refuge (AR)"]
  )
...
species_final_pt_1 <- species_final_pt_1 %>%
  mutate(
    rate_per_cam_day_hibernacula = count_observed_at_artificial_refuge_ar / camera_days_vec_pt_1["Artificial Refuge (AR)"]
  )
...
t1_pt1 = species_final_pt_1 %>%
  ...
  add_header_above(c(" " = 1, "Artificial Refuge (AR)" = 3))
...
species_final_hourly <- species_combined_hourly %>%
  mutate(
    percent_hibernacula_visited = 100 * features_visited_at_artificial_refuge_ar / total_features_hourly_vec["Artificial Refuge (AR)"],
...
species_final_hourly <- species_final_hourly %>%
  mutate(
    rate_per_cam_day_hibernacula = count_observed_at_artificial_refuge_ar / camera_days_vec_hourly["Artificial Refuge (AR)"],
...
t1_hourly <- species_final_hourly %>%
  ...
  mutate(
    'Detections   ' = Detections + `Detections ` + `Detections  `,
    '% Visited   ' = (
      (`% Visited` * total_features_hourly_vec["Artificial Refuge (AR)"]) +
...
species_final_hourly <- species_combined_hourly %>%
  mutate(
    ...
    percent_hibernacula_visited = 100 * safe_div(
      features_visited_at_artificial_refuge_ar, total_features_artificial_refuge_ar
    ),
    ...
    rate_per_cam_day_hibernacula = safe_div(
      count_observed_at_artificial_refuge_ar, cam_days_artificial_refuge_ar
    ),
    ...
    detections_total = rowSums(across(c(count_observed_at_artificial_refuge_ar,
                                        count_observed_at_boulder,
                                        count_observed_at_log)), na.rm = TRUE),
    total_features_all = rowSums(across(c(total_features_artificial_refuge_ar,
                                          total_features_boulder,
                                          total_features_log)), na.rm = TRUE),
    total_cam_days_all = rowSums(across(c(cam_days_artificial_refuge_ar,
                                          cam_days_boulder,
                                          cam_days_log)), na.rm = TRUE),

    percent_visited_all = 100 * safe_div(
      rowSums(across(c(features_visited_at_artificial_refuge_ar,
...
metrics_long <- species_final_hourly %>%
  transmute(
    ...
    # Artificial Refuge (AR)
    detections_hib = count_observed_at_artificial_refuge_ar,
...
build_feature_spans <- function(part) {
  out <- c()
  n <- count_for(part, "hib"); if (n > 0) out <- c(out, "Artificial Refuge (AR)" = n)
