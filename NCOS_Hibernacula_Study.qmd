...
# Load NCOS habitat layer from ArcGIS Online
habitat_url <- "https://services1.arcgis.com/4TXrdeWh0RyCqPgB/arcgis/rest/services/NCOS_Habitats_June2021_Simple/FeatureServer/0/query?where=1%3D1&outFields=*&f=geojson"
ncos_habitats <- st_read(habitat_url, quiet = TRUE)
# Transform to WGS84 to match other layers
...
map_main <- ggplot() +
  ...
  scale_fill_identity() +
  # Add habitat layer
  geom_sf(data = ncos_habitats, aes(fill = after_scale(alpha("white", 0.3))),
          color = "yellow", linewidth = 0.8, show.legend = FALSE) +
...
# Load NCOS habitat layer from ArcGIS Online (if not already loaded)
if (!exists("ncos_habitats")) {
  habitat_url <- "https://services1.arcgis.com/4TXrdeWh0RyCqPgB/arcgis/rest/services/NCOS_Habitats_June2021_Simple/FeatureServer/0/query?where=1%3D1&outFields=*&f=geojson"
  ncos_habitats <- st_read(habitat_url, quiet = TRUE)
  ncos_habitats <- st_transform(ncos_habitats, crs = 4326)
}
