...
if (dispersion_ratio > 1) {
  cat("Dispersion ratio indicates overdispersion:", dispersion_ratio, "\n")
} else {
  cat("Dispersion ratio is acceptable:", dispersion_ratio, "\n")
}
```
...
species_counts <- merged_data_with_combined_observations %>%
  filter(!is.na(common_name)) %>%
  count(common_name, name = "total_count")

# Recode species names based on detection threshold or name content
merged_data_lumped <- merged_data_with_combined_observations %>%
...
basemap <- get_tiles(site_sf, provider = "Esri.WorldImagery", crop = TRUE, zoom = 16)
...
final_map <- ggdraw() +
  draw_plot(main_map) +
  draw_plot(overview_map, x = 0.7, y = 0.65, width = 0.28, height = 0.28)
