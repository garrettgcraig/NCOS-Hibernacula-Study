...
stargazer(quasi_model_1, quasi_model_2, quasi_model_3,
          ...
          dep.var.labels = "Daily Total Observations",
          column.labels = c("Model 1", "Model 2", "Model 3"),
          covariate.labels = c("Feature Type", "Trail", "Habitat Type"),
          digits = 3)
```
...
The Welch two-sample t-test comparing average daily wildlife observations between Constructed Hibernacula and Log/Boulder features showed the following:

- The test was **not statistically significant**, with a p-value of `r p_value_text`.
- On average, Constructed Hibernacula had `r mean_hibernacula` daily observations, while Log/Boulder features had `r mean_log_boulder`.
- The mean difference in average daily observations was `r mean_difference`, with a 95% confidence interval ranging from `r ci_lower` to `r ci_upper`.

Though the p-value exceeds the conventional significance threshold (0.05), this does not rule out a meaningful difference.

Supporting this, the effect size measured by Cohen's d was `r cohen_d_value`, which corresponds to a `r cohen_d_magnitude` effect size.

The 95% confidence interval for Cohen's d ranged from `r cohen_d_ci_lower` to `r cohen_d_ci_upper`, which includes zero, suggesting some uncertainty about the direction and magnitude of the effect.

Assumption checks for the t-test included:

- The Shapiro-Wilk test for normality found p-values of `r shapiro_p_hibernacula` for Constructed Hibernacula and `r shapiro_p_log_boulder` for Log/Boulder sites. Both p-values are greater than 0.05, so the data do not strongly violate normality assumptions.

- Levene’s test indicated unequal variances (F = `r levene_f_value`, p = `r levene_p_value`), confirming heteroscedasticity between groups.

Despite unequal variances, the Welch t-test remains appropriate due to its robustness to variance heterogeneity and moderate sample sizes.
...
Sites with Boulder/Log features have ~9.4 fewer daily observations compared to the reference feature type.
This effect is statistically significant at p < 0.01.

**Model 2:**
Adds Trail (Yes/No):
Boulder/Log effect remains significant and grows stronger (~11 fewer observations).
The presence of a Trail reduces observations by ~4.9, but this effect is not statistically significant.

**Model 3:**
Adds Habitat Type (with Marsh as the reference):
Boulder/Log still shows a strong, significant negative effect (~11 fewer observations).
Trail effect becomes slightly stronger (~7 fewer observations) but is still not significant.
Grassland habitat appears to increase observations by ~5.8, and Scrub by ~2.7, but neither are statistically significant.

**Model Fit (R²):**
Very low R² across models (~7–10%), meaning the linear model explains only a small portion of the variance in daily observations.

#### Poisson GLMs

These models are more appropriate for count data like daily observations. The coefficients are on the log scale, so interpretation requires exponentiation.

**Model 1:**
Boulder/Log sites have a log count decrease of 0.761, or about 53% fewer expected daily observations (exp(-0.761) ≈ 0.467).
Highly significant (p < 0.01).

**Model 2:**
Adding Trail:
Trail presence decreases expected counts by about 26% (exp(-0.297) ≈ 0.743), significant at p < 0.01.
Boulder/Log effect remains large and significant.

**Model 3:**
Adding Habitat Type:
Grassland: +56% increase (exp(0.447) ≈ 1.564), significant.
Scrub: +25% increase (exp(0.226) ≈ 1.253), significant.
Trail and Boulder/Log still show strong, significant negative effects.

**Model Fit:**
Log-likelihood improves across models (i.e., fit improves).
AIC decreases across models (lower = better).

**BUT:**
Overdispersion is present in all Poisson models (dispersion ratio ~13.9), which violates Poisson assumptions and may lead to underestimated standard errors.

#### Quasi-Poisson GLMs

These models correct for overdispersion by adjusting standard errors (coefficients are the same as Poisson).
Coefficients remain unchanged.
Standard errors are larger, leading to loss of statistical significance for:
- Trail effect (now non-significant).
- Habitat type (now non-significant).
- Feature type remains significant (p < 0.01) in all models.
...
Trail presence and Habitat Type effects appear meaningful in the Poisson models, but are not robust under corrected standard errors (quasi-Poisson).

Poisson models fit the data better than linear models, but overdispersion must be addressed.

Quasi-Poisson corrects for this, indicating that only the Boulder/Log effect is statistically robust.
...
main_map <- ggplot() +
  ...
  scale_fill_identity() +
  geom_sf(data = site_sf, aes(color = feature_type_methodology, size = avg_daily_observations), alpha = 0.9) +
...
# Overview map of CA with red dot
...
overview_map <- ggplot() +
  geom_sf(data = ca_map, fill = "gray90", color = "gray40") +
  geom_point(data = centroid_coords, aes(x = longitude, y = latitude), color = "red", size = 2) +
  theme_void()

# Combine with cowplot
final_map <- ggdraw() +
  draw_plot(main_map) +
  draw_plot(overview_map, x = 0.7, y = .5, width = 0.28, height = 0.28)
