...
deployment_summary <- merged_sequences_and_deployment_data %>%
  group_by(deployment_id, placename, feature_type_methodology, latitude, longitude, start_date, end_date, deployment_duration) %>%
  summarize(
...
ggplot(deployment_summary_recoded, aes(x = feature_type_methodology, y = avg_daily_observations, fill = feature_type_methodology)) +
  ...
  stat_summary(fun = "mean", geom = "point", shape = 18, size = 3, color = "white") +  # Mean as white diamonds
